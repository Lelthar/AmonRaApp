...
export class ViromediaController extends Component {
  ...
  constructor(props) {
    console.log("Constructor ViromediaController")
    super(props);
  ...
  render() { 
    if (this.state.navigatorType == NAVIGATOR_TYPE_AR && this.state.focusedScreen){
      return <ARController sharedProps={this.state.sharedProps}/>

    } else if (this.state.navigatorType == NAVIGATOR_TYPE_VR) {
      return <VRController sharedProps={this.state.sharedProps} content={this.state.content}/>

    } else if (this.state.navigatorType == NAVIGATOR_TYPE_3D) {
      return <D3Controller sharedProps={this.state.sharedProps} content={this.state.content}/>
